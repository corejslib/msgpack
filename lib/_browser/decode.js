function utf8Read ( view, offset, length ) {
    ...
    for ( var i = offset, end = offset + length; i < end; i++ ) {
        ...
        if ( ( byte & 0xF0 ) === 0xE0 ) {
            string += String.fromCodePoint( ( ( byte & 0x0F ) << 12 ) | ( ( view.getUint8( ++i ) & 0x3F ) << 6 ) | Math.trunc( view.getUint8( ++i ) & 0x3F ) );

            continue;
        }

        if ( ( byte & 0xF8 ) === 0xF0 ) {
            chr = ( ( byte & 0x07 ) << 18 ) | ( ( view.getUint8( ++i ) & 0x3F ) << 12 ) | ( ( view.getUint8( ++i ) & 0x3F ) << 6 ) | Math.trunc( view.getUint8( ++i ) & 0x3F );

            if ( chr >= 0x010000 ) {

                // surrogate pair
                chr -= 0x010000;
                string += String.fromCodePoint( ( chr >>> 10 ) + 0xD800, ( chr & 0x3FF ) + 0xDC00 );
            }
            else {
